...
function AktivitasBerakhir({ loading, dataAktivitas, activeIndex }) {
  const isOnline = useOnlineStatus()
  const today = new Date()

  //   Filter aktivitas yang telah berakhir
  const endActivities =
    dataAktivitas &&
    dataAktivitas.filter((activity) => {
                          ...
                          <div className="flex">
                            ...
                            {dayToGo(value.batas_waktu) !== 'Berakhir' ? (
                              <p className="text-sm text-peduly-dark ml-[7px] leading-6">
                                Batas Waktu {dayToGo(value.batas_waktu)} lagi{' '}
                              </p>
                            ) : (
                              <p className="text-sm text-peduly-dark ml-[7px] leading-6">
                                Batas Waktu {dayToGo(value.batas_waktu)}
                              </p>
                            )}
                          </div>
...
export default AktivitasBerakhir
