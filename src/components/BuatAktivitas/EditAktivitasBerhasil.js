...
import {
  getEditAktivitasBerhasil,
  removeEditAktivitasBerhasil,
} from 'utils/cookiesHooks'

const EditAktivitasBerhasil = () => {
  const history = useHistory()

  useEffect(() => {
    if (getEditAktivitasBerhasil() === 'sukses') {
      removeEditAktivitasBerhasil()
    }
  }, [])

  if (!getEditAktivitasBerhasil()) {
    return <Redirect to="/" />
  }
...
export default EditAktivitasBerhasil
