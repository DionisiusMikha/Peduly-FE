...
import { getIdTransaksi, getToken, removeIdTransaksi, setPartisipasiBerhasil } from 'utils/cookiesHooks'
...
const DetailPembayaran = () => {
  ...
  const { slug, idTransaksi } = useParams()
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(`${API_URL}/api/donation/transaction/${idTransaksi}`);
      setStatus(response.data.data.status_donasi);
      ...
      if (status === 'Approved') {
        ...
        history.push({
          ...
          state: {
            ...
            P1: `Cek email ${userData.user.email} untuk proses selanjutnya.`,
            hrf: 'profile',
            Button: 'Kembali',
          },
        })
  ...
  const [timeRemaining, setTimeRemaining] = useState(formatTime);

  useEffect(() => {
    const timerId = setInterval(() => {
      setTimeRemaining(formatTime()); // Panggil formatTime() untuk mendapatkan waktu terkini
    }, 1000);
  ...
  function formatTime() {
    // Pastikan dataTransaksi.deadline dan dataTransaksi.created_at memiliki format tanggal yang sesuai
    const firstDate = new Date(dataTransaksi.deadline);
  ...
  const getDetailTransaksi = async () => {
    ...
    if (!idTransaksi) return
    await fetcher(`api/donation/transaction/${idTransaksi}`)
      // await fetcher(`api/donation/transaction/${idTranksaksi}`)
      .then(
        (res) => setDataTransaksi(
  ...
  useEffect(() => {
    if (idTransaksi)
      getDetailTransaksi()
  }, [idTransaksi]) // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="relative">
          ...
          <div className="w-full mr-[20px] ml-[13px]">
            ...
            <div className="flex justify-between text-sm font-semibold">
              ...
              <span className="flex items-center">
                ...
                <div>
                  {timeRemaining.hours + timeRemaining.minutes + timeRemaining.seconds >= 0 ? (
                    <p className="text-basic font-bold ml-1">
                      {timeRemaining.hours}:{timeRemaining.minutes}:{timeRemaining.seconds}
                    </p>
                  ) : (
                    <p className="text-basic font-bold ml-1">--:--:--</p>
                  )}
                </div>
              </span>
            </div>
        ...
        <div className="rounded-t-[30px] bg-white  w-full relative -top-7 shadow-sm">
          ...
          {/* <div className="mx-[20px]">
            <Link
              to={`/donasi-sekali/${slug}/pembayaran/instruksi?idn=${getIdTransaksi()}`}
            >
          ...
          {getToken() && (
            <>
              <hr className="mt-[24px] mx-[40px] border-involuntir-light" />
              <div
                className="flex justify-center py-[16px] cursor-pointer"
                onClick={() => {
                  history.push({
                    pathname: `/aktivitas/${slug}/pembayaran/status/${dataTransaksi.id}`,
                    state: {
                      id: dataTransaksi.id
                    }
                  })
              ...
              <hr className="mx-[40px] border-involuntir-light" />
            </>
          )}
          {/* donasi terlindungi  */}
          ...
          {dataTransaksi.metode_pembayaran === 'emoney' && (
            <div className="px-5 flex flex-col w-full">
              <hr className="border-t-[1px] border-none border-involuntir-light-1/2 mb-3 mt-[34px]" />
